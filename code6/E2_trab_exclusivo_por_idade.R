...
d = espetaculo %>% select(voce_trabalha_exclusivamente_na_area_cultural,faixa_idade)

d = d %>%   group_by(faixa_idade,voce_trabalha_exclusivamente_na_area_cultural) %>%
  summarise(n = n()) %>%
  mutate(freq = n / sum(n))
...
g1 = ggplot(d) +
  aes(x = faixa_idade, y = freq, fill = voce_trabalha_exclusivamente_na_area_cultural) +
  geom_col() +
...
e = visuais %>% select(voce_trabalha_exclusivamente_na_area_cultural,faixa_idade)

e = e %>%   group_by(faixa_idade,voce_trabalha_exclusivamente_na_area_cultural) %>%
  summarise(n = n()) %>%
  mutate(freq = n / sum(n))
...
ggsave("graficos_v6/entrega2_trabalho_exclusivo_idade.png",width = 20, height = 14, units = "cm")
remove(d,e) 
