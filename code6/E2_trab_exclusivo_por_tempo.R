...
d = espetaculo %>% select(voce_trabalha_exclusivamente_na_area_cultural,ha_quanto_tempo_voce_atua_nessa_area)

d = d %>% group_by(ha_quanto_tempo_voce_atua_nessa_area,voce_trabalha_exclusivamente_na_area_cultural) %>%
  summarise(n = n()) %>%
  mutate(freq = n / sum(n))
...
g1 = ggplot(d) +
  aes(x = ha_quanto_tempo_voce_atua_nessa_area, y = freq, fill = voce_trabalha_exclusivamente_na_area_cultural) +
  geom_col() +
...
e = visuais %>% select(voce_trabalha_exclusivamente_na_area_cultural,ha_quanto_tempo_voce_atua_nessa_area)

e = e %>% group_by(ha_quanto_tempo_voce_atua_nessa_area,voce_trabalha_exclusivamente_na_area_cultural) %>%
  summarise(n = n()) %>%
  mutate(freq = n / sum(n))
...
annotate_figure(figure,
                ...
                bottom = text_grob("Há quanto tempo você atua nessa área? \n  Fonte: ECOA", color = "#b02c57",
...
ggsave("graficos_v6/entrega2_trabalho_exclusivo_tempo.png",width = 20, height = 14, units = "cm")
remove(d,e) 
